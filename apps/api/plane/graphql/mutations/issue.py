...
def get_feature_flag(workspace_slug: str, user_id: str, flag_key: str):
    from plane.payment.contract_license import get_contract_feature_flags, is_contract_license_enabled

    base = getattr(settings, "FEATURE_FLAG_SERVER_BASE_URL", "") or ""
    token = getattr(settings, "FEATURE_FLAG_SERVER_AUTH_TOKEN", "") or ""
    if base and token and not is_contract_license_enabled():
        try:
            url = f"{base}/api/feature-flags/"
            payload = {
    ...
    flags = list(get_contract_feature_flags().values())[0]
