...
def get_feature_flag_server_base_url():
    base_url = settings.FEATURE_FLAG_SERVER_BASE_URL or settings.PAYMENT_SERVER_BASE_URL
...
def fetch_feature_flags(slug: str, user_id: strawberry.ID):
    ...
    base_url = get_feature_flag_server_base_url()
    if not base_url:
        return get_contract_feature_flags().values()
