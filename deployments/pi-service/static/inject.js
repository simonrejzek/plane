/**
 * Cosmic inject v30 — minimal. Site must load first.
 * No service workers. No reloads. No import maps.
 */
(function () {
  ...
  // Soft-force critical flags only (does not rewrite JS modules)
  (function forceFlags() {
    function kindOf(url) {
      if (!url) return null;
      var u = String(url);
    ...
    function patch(kind, data) {
      try {
        ...
        if (kind === "auth") { data.is_authorized = true; return data; }
        if (kind === "features") {
          data.is_pi_enabled = true; data.is_wiki_enabled = true; return data;
        }
        if (kind === "flags") {
        ...
        if (k) {
          x.addEventListener("readystatechange", function () {
            ...
            try {
              ...
              try { Object.defineProperty(x, "responseText", { get: function () { return t; } }); } catch (_) {}
              try { Object.defineProperty(x, "response", { get: function () { return t; } }); } catch (_) {}
            } catch (_) {}
          });
        }
